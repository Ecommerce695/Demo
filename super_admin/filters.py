...
from .serializers import SuperAdminProductsFilter, SuperAdminVendors_AdminFilter, SuperAdminOrderFilter, SuperAdminUsersFilter, SuperAdminSalesFilter
...
class SAProductsIncludingFilters(APIView):
    serializer_class = SuperAdminProductsFilter
...
class SAGetAllProductsIncludingFilters(CreateAPIView):
    serializer_class = SuperAdminProductsFilter
...
class SAGetUsersListIncludingFilters(CreateAPIView):
    serializer_class = SuperAdminUsersFilter
                                ...
                                if user_list.exists():
                                    ...
                                    for j in user_list:
                                        if (UserRole.objects.filter(user_id=j['id'],role_id=3)):
                                            ...
                                            datalist.append(data)
                                        elif (UserRole.objects.filter(user_id=j['id'],role_id=2)):
                                            assigned_role = 'ADMIN'
                                            usertable= UserProfile.objects.get(id = j['id'])
...
class SAGetVendorsListIncludingFilters(CreateAPIView):
    serializer_class = SuperAdminVendors_AdminFilter
    ...
    def post(self,request,token):
            ...
            if(UserProfile.objects.filter(id=user, is_active='True')):
                if token1.expiry < datetime.now(utc):
                    ...
                    if serializer.is_valid(raise_exception=True):
                        ...
                        # tax=serializer.data['tax_id']
                        tax_s=serializer.data['tax_status']
                        email=serializer.data['email']
                        date1=serializer.data['from_date']
                        date2=serializer.data['to_date']
                        pageno=serializer.validated_data['pageno']
                        ...
                        if active!='':
                            user_list=CompanyProfile.objects.filter(is_active__icontains=active,org_name__icontains=name,tax_status__icontains=tax_s,email__icontains=email,date_joined__range=(d1,d2+timedelta(days=1))).values().order_by('-date_joined','-id') 
                        else:
                            user_list=CompanyProfile.objects.filter(org_name__icontains=name,tax_status__icontains=tax_s,email__icontains=email,date_joined__range=(d1,d2+timedelta(days=1))).values().order_by('-date_joined','-id')
                        if d1<=d2:
                            l=[]                   
                            if user_list.exists():
                                datalist =[]
        ...
        return Response({'message':"Current User is not Super Admin"},status=status.HTTP_404_NOT_FOUND)


class SAAllGetOrdersListIncludingFilters(CreateAPIView):
    serializer_class = SuperAdminOrderFilter
        ...
        if role.exists():
            if(token1.expiry < datetime.now(utc)):
                ...
                if serializer.is_valid(raise_exception=True):
                    ...
                    if d1<=d2:
                        if p_status=='':
                            o = OrderItemHistory.objects.filter(alias__icontains=o_id,shipment_status__icontains=shipment_status, order_status__icontains=o_status,created_at__range=(d1,d2+timedelta(days=1))).values().order_by('-created_at','-id') #and OrderItemHistory.objects.filter(alias__icontains=o_id,order_status__icontains=o_status,created_at__lte=d2).values()
                            list=[]
                            for i in o:
                            ...
                            if tt.exists():
                                list=[]
                                for t in tt:
                                    o = OrderItemHistory.objects.filter(id=int(t['orderitem']),alias__icontains=o_id,shipment_status__icontains=shipment_status,order_status__icontains=o_status,created_at__range=(d1,d2+timedelta(days=1))).values().order_by('-created_at') #and OrderItemHistory.objects.filter(id=int(t['orderitem']),alias__icontains=o_id,order_status__icontains=o_status,created_at__lte=d2).values()
                                    for i in o:
                                        orders = OrderItemHistory.objects.get(alias=i['alias'])
                                        ...
                                        list.append(data)
                                paginator = Paginator(list,prperpage)
                                page = request.GET.get("page",pageno)
                                object_list = paginator.page(page)
                                a=list(object_list)
                                data1 = {
...
class SAGetSalesListIncludingFilters(CreateAPIView):
    serializer_class = SuperAdminSalesFilter
                ...
                if serializer.is_valid(raise_exception=True):
                    ...
                    if d1<=d2:
                        ...
                        if (Transaction_table.objects.filter(status='PAID').exists()):
                            if i_id=='':
                                tx=Transaction_table.objects.filter(orderitem=o_id[0],alias__icontains=t_id,created_at__range=(d1,d2+timedelta(days=1))).values().order_by('-created_at','-id') 
                                datalist=[]
                                for k in tx:
                                    d = OrderItemHistory.objects.filter(id=k['orderitem'],shipment_status=d_status).values()
                                    for i in d:
                                        try:
                                ...
                                p=Payment_details_table.objects.filter(invoice__icontains=i_id).values()
                                l=[]
                                if p.exists():
                                    datalist=[]
                                    for i in p:
...
class SAGetAdminsListIncludingFilters(CreateAPIView):
    serializer_class = SuperAdminVendors_AdminFilter
                    ...
                    if serializer.is_valid(raise_exception=True):
                        ...
                        if d1<=d2:    
                            ...
                            if user_list.exists():
                                datalist =[]
                                for j in user_list:
                                    if (UserRole.objects.filter(user_id=j['user_id'],role_id=2).exists()):
                                        assigned_role = 'ADMIN'
                                        admin = CompanyProfile.objects.get(user=j['user_id'])
                                        data = {
                                        "org_name" : admin.org_name,
                                        "mobile" : admin.mobile,
                                        "email" : admin.email,
                                        "tax_id" : admin.tax_id,
                                        "tax_status": admin.tax_status,
                                        "is_active" : admin.is_active,
                                        "date_joined":admin.date_joined.date(),
                                        "role":assigned_role,
                                        }
                                        datalist.append(data)
                                paginator = Paginator(datalist,prperpage)
